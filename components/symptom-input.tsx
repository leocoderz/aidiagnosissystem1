...
import { Plus, Trash2, Clock, MapPin, Mic, Brain, Stethoscope, AlertTriangle } from "lucide-react"
...
export default function SymptomInput({
  ...
  const handleGetDiagnosis = () => {
    if (symptoms.length === 0) {
      toast({
        ...
        description: "Please add at least one symptom before requesting diagnosis",
        variant: "destructive",
      })
      return
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-xl">Add New Symptom</CardTitle>
          <CardDescription>Record your symptoms for AI-powered medical analysis</CardDescription>
        </CardHeader>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-xl">Your Symptoms ({symptoms.length})</CardTitle>
              <CardDescription>Review and manage your recorded symptoms</CardDescription>
            </div>
            {symptoms.length > 0 && (
              <Button
        ...
        <CardContent>
          {symptoms.length === 0 ? (
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-2">No symptoms recorded yet</h3>
              <p className="text-gray-600">Add your first symptom above to get started with AI diagnosis</p>
            </div>
          ) : (
            <div className="space-y-4">
              {symptoms.map((symptom, index) => (
                <div key={symptom.id} className="p-4 border rounded-lg bg-gray-50 dark:bg-gray-800">
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-3">
                        <h4 className="font-semibold text-lg">{symptom.name}</h4>
                        <Badge className={`${getSeverityColor(symptom.severity)} border-0`}>
                      ...
                      <p className="text-xs text-gray-400">Added: {new Date(symptom.timestamp).toLocaleString()}</p>
                    </div>
              ...
              <div className="bg-blue-50 dark:bg-blue-950 p-4 rounded-lg">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <h4 className="font-semibold text-blue-900 dark:text-blue-100 mb-1">AI Medical Analysis Ready</h4>
                    <p className="text-sm text-blue-700 dark:text-blue-200">
                      You have {symptoms.length} symptom{symptoms.length !== 1 ? "s" : ""} recorded. Click "Get AI
                      Diagnosis" to receive comprehensive medical analysis, potential conditions, and treatment
